...
export class Activation {
	...
	private hasDifferentSiteIdForSameAwardScheme( spot: Spot ): boolean {
		...
		return this.awardList.filter( v=> v.award == spot.award && v.siteId != spot.siteId ).length > 0;
	...
	private extractIdentifyingLocationWords( spot: Spot ): string[] {
		const commonLocationWords = ["park", "area", "recreation", "reserve", "nature", "summit", "hill", "mountain", "mt",  "lake", "pond", "river", "creek", "stream", "trail", "road", "drive", "lane", "highway", "route", "path", "way", "avenue", "street", "place", "court", "circle", "boulevard", "parkway", "parking", "lot", "campground", "camp", "site", "beach", "island", "islet", "rock", "cave", "cavern", "canyon", "cliff", "bluff", "ridge", "peak", "mount", "mountain", "hill", "summit", "butte", "knob", "dome", "volcano", "crater", "glacier", "forest", "woods", "grove", "jungle", "swamp", "marsh", "bog", "wetland", "desert", "dune", "oasis", "plain", "prairie", "savanna", "tundra", "valley", "wadi", "waterfall", "rapids", "spring", "geyser", "hot", "spring", "geothermal", "spring", "geyser", "hot", "spring", "geothermal", "spring", "geyser", "hot", "spring", "geothermal", "spring", "geyser", "hot", "spring", "geothermal", "spring", "geyser", "hot", "spring", "geothermal", "spring", "geyser", "hot", "spring", "geothermal", "spring", "geyser", "hot", "spring", "geothermal", "spring", "geyser", "hot", "spring", "geothermal", "spring", "geyser", "hot", "spring", "geothermal", "spring"];

		const words = spot.siteName.split(" ").filter( v=> commonLocationWords.indexOf( v.toLowerCase() ) == -1 );
	...
	private setSpotType(spot: Spot): void {
		...
		console.log("length: " + this._spotList.length);
		console.log("index: " + index);
		if( index > 0 ) {
			const previousSpot = this._spotList[index - 1];
			console.log("previous spot: " + previousSpot.type);
			console.log("Modes match: " + (previousSpot.mode == spot.mode));
			console.log("Freqs match: " + (previousSpot.frequency == spot.frequency));
			console.log("Both Match" + (previousSpot.mode == spot.mode && previousSpot.frequency == spot.frequency));
			spot.type = previousSpot.mode == spot.mode && previousSpot.frequency == spot.frequency 
				? SpotType.Respot 
				: SpotType.Spot;
		}
		else {
...
export class ActivationAward {
	...
	constructor( public award: AwardScheme, public siteId: string ) {
